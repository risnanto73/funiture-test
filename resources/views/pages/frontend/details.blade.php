    ...
    <section class="container mx-auto">
        <div class="flex flex-wrap my-4 md:my-12">
            <div class="w-full md:hidden px-4">
                <h2 class="text-5xl font-semibold">{{ $produk->name }}</h2>
                <span class="text-xl">IDR {{ number_format($produk->price) }}</span>
            </div>
            <div class="flex-1">
                <div class="slider">
                    <div class="thumbnail">
                        @foreach ($produk->galleries as $item)
                            <div class="px-2">
                    ...
                    <div class="preview">
                        <div class="item rounded-lg h-full overflow-hidden">
                            <img src="{{ $item->exists() ? Storage::url($item->first()->url) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
            ...
            <div class="flex-1 px-4 md:p-6">
                <h2 class="text-5xl font-semibold">{{ $produk->name }}</h2>
                <p class="text-xl">IDR {{ number_format($produk->price) }}</p>
                    ...
                    <div class="px-3 flex-none" style="width: 320px">
                        <div class="rounded-xl p-4 pb-8 relative bg-white">
                            <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                <img src="{{ $item->galleries()->exists() ? Storage::url($item->galleries->first()->url) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
